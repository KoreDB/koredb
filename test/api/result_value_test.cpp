...
static decltype(auto) copyFlatTuple(koredb::processor::FlatTuple* tuple) {
    std::vector<std::unique_ptr<koredb::common::Value>> ret;
...
TEST_F(ResultValueTest, getNextExample) {
    std::unique_ptr<koredb::main::QueryResult> result = conn->query("MATCH (p:person) RETURN p.*");
    std::vector<std::vector<std::unique_ptr<koredb::common::Value>>> tuples;
