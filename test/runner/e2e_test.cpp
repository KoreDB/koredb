...
public:
    ...
    void SetUp() override {
        ...
        if (datasetType == TestGroup::DatasetType::KOREDB && dataset != "empty" &&
            !generateBinaryDemo) {
        ...
        if (datasetType != TestGroup::DatasetType::KOREDB && dataset != "empty") {
            ...
            initGraph(TestHelper::appendKoreDBRootPath("dataset/demo-db/parquet/"));
        } else if (generateBinaryDemo) {
            initGraph(TestHelper::appendKoreDBRootPath(
    ...
    void setUpDataset() {
        ...
        case TestGroup::DatasetType::CSV_TO_PARQUET: {
            auto csvDatasetPath = TestHelper::appendKoreDBRootPath("dataset/" + dataset);
        ...
        default: {
            ...
            // datasets exported from earlier KoreDB versions, otherwise the default path.
            std::string rootDir = TestHelper::E2E_OVERRIDE_IMPORT_DIR.empty() ?
                                      "dataset/" :
                                      TestHelper::E2E_OVERRIDE_IMPORT_DIR;
            dataset = TestHelper::appendKoreDBRootPath(rootDir + dataset);
        }
        }
    }
...
void checkGtestParams(int argc, char** argv) {
    if (argc > 1) {
        ...
        if (argument == "--gtest_list_tests") {
            ...
            scanTestFiles(TestHelper::appendKoreDBRootPath(TestHelper::E2E_TEST_FILES_DIRECTORY));
        }
        if (argument.starts_with("--gtest_filter=")) {
            std::string testCaseFile = findTestFile(argument.substr(15));
            if (testCaseFile.empty()) {
                scanTestFiles(
                    TestHelper::appendKoreDBRootPath(TestHelper::E2E_TEST_FILES_DIRECTORY));
            } else {
...
int main(int argc, char** argv) {
    try {
        ...
        if (env_import_data_dir != nullptr) {
            auto path = TestHelper::appendKoreDBRootPath(
                std::filesystem::path(env_import_data_dir).string());
