...
public:
    ...
    static constexpr uint64_t HASH_INDEX_MEM =
        common::HashIndexConstants::NUM_HASH_INDEXES * common::KOREDB_PAGE_SIZE * 9;
    ...
    static std::string getTestListFile() {
        return appendKoreDBRootPath(std::string(E2E_TEST_FILES_DIRECTORY) + "/test_list");
    }

    static std::string appendKoreDBRootPath(const std::string& path) {
        if (std::filesystem::path(path).is_relative()) {
            return KOREDB_ROOT_DIRECTORY + std::string("/") + path;
        }
        return path;
    }
    ...
    static std::filesystem::path getRootTempDir() {
        ...
        if (tempDir != nullptr) {
            return std::filesystem::path(tempDir) / "koredb";
        }
        return std::filesystem::temp_directory_path() / "koredb";
    }
...
#ifdef _WIN32
        // koredb still doesn't support backslashes in paths on windows
