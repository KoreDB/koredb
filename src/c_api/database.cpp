...
koredb_state koredb_database_init(const char* database_path, koredb_system_config config,
    koredb_database* out_database) {
    try {
...
void koredb_database_destroy(koredb_database* database) {
    if (database == nullptr) {
        return;
    }
...
koredb_system_config koredb_default_system_config() {
    SystemConfig config = SystemConfig();
    auto cSystemConfig = koredb_system_config();
