...
public:
     ...
     */
    KOREDB_API explicit Connection(Database* database);
    /**
     * @brief Destructs the connection.
     */
    KOREDB_API ~Connection();
    /**
     ...
     */
    KOREDB_API void setMaxNumThreadForExec(uint64_t numThreads);
    /**
     ...
     */
    KOREDB_API uint64_t getMaxNumThreadForExec();
     ...
     */
    KOREDB_API std::unique_ptr<QueryResult> query(std::string_view query);
     ...
     */
    KOREDB_API std::unique_ptr<PreparedStatement> prepare(std::string_view query);
     ...
     * @return the prepared statement.
     */
    KOREDB_API std::unique_ptr<PreparedStatement> prepareWithParams(std::string_view query,
     ...
     * @return the result of the query.
     */
    KOREDB_API std::unique_ptr<QueryResult> executeWithParams(PreparedStatement* preparedStatement,
        std::unordered_map<std::string, std::unique_ptr<common::Value>> inputParams);
    /**
     * @brief interrupts all queries currently executing within this connection.
     */
    KOREDB_API void interrupt();

    /**
     * @brief sets the query timeout value of the current connection. A value of zero (the default)
     * disables the timeout.
     */
    KOREDB_API void setQueryTimeOut(uint64_t timeoutInMS);
...
private:
    ...
    KOREDB_API void addScalarFunction(std::string name, function::function_set definitions);
    KOREDB_API void removeScalarFunction(std::string name);
