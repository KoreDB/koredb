...
enum class KOREDB_API DatePartSpecifier : uint8_t {
    YEAR,
    MONTH,
    DAY,
    DECADE,
...
public:
    ...
    KOREDB_API static void addition(interval_t& result, uint64_t number, std::string specifierStr);
    KOREDB_API static interval_t fromCString(const char* str, uint64_t len);
    KOREDB_API static std::string toString(interval_t interval);
    KOREDB_API static bool greaterThan(const interval_t& left, const interval_t& right);
    KOREDB_API static void normalizeIntervalEntries(interval_t input, int64_t& months,
        int64_t& days, int64_t& micros);
    KOREDB_API static void tryGetDatePartSpecifier(std::string specifier,
        DatePartSpecifier& result);
    KOREDB_API static int32_t getIntervalPart(DatePartSpecifier specifier, interval_t timestamp);
    KOREDB_API static int64_t getMicro(const interval_t& val);
    KOREDB_API static int64_t getNanoseconds(const interval_t& val);
    KOREDB_API static const regex::RE2& regexPattern1();
    KOREDB_API static const regex::RE2& regexPattern2();
};
