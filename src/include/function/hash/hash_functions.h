...
inline void Hash::operation(const std::string_view& key, common::hash_t& result) {
    ...
    for (size_t i = 0u; i < key.size() / 8; i++) {
        auto blockHash = koredb::function::murmurhash64(*(data64 + i));
        hashValue = koredb::function::combineHashScalar(hashValue, blockHash);
    }
    uint64_t last = 0;
    for (size_t i = 0u; i < key.size() % 8; i++) {
    ...
    hashValue =
        koredb::function::combineHashScalar(hashValue, koredb::function::murmurhash64(last));
    result = hashValue;
}
