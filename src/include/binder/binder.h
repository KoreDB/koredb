...
public:
    ...
    KOREDB_API std::unique_ptr<BoundStatement> bind(const parser::Statement& statement);
    ...
    KOREDB_API std::shared_ptr<Expression> createVariable(const std::string& name,
        const common::LogicalType& dataType);
    KOREDB_API std::shared_ptr<Expression> createInvisibleVariable(const std::string& name,
        const common::LogicalType& dataType) const;
    KOREDB_API expression_vector createVariables(const std::vector<std::string>& names,
        const std::vector<common::LogicalType>& types);
    KOREDB_API expression_vector createInvisibleVariables(const std::vector<std::string>& names,
    ...
    KOREDB_API std::shared_ptr<NodeExpression> createQueryNode(const std::string& parsedName,
        const std::vector<catalog::TableCatalogEntry*>& entries);
    ...
    /*** validations ***/
    KOREDB_API static void validateTableExistence(const main::ClientContext& context,
        const std::string& tableName);
    KOREDB_API static void validateNodeTableType(const catalog::TableCatalogEntry* entry);
    KOREDB_API static void validateColumnExistence(const catalog::TableCatalogEntry* entry,
    ...
    KOREDB_API void addToScope(const std::string& name, std::shared_ptr<Expression> expr);
