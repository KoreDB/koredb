...
public:
    ColumnReader(ParquetReader& reader, common::LogicalType type,
        const koredb_parquet::format::SchemaElement& schema, common::idx_t fileIdx,
    ...
    static std::unique_ptr<ColumnReader> createReader(ParquetReader& reader,
        common::LogicalType type, const koredb_parquet::format::SchemaElement& schema,
    ...
    void decompressInternal(koredb_parquet::format::CompressionCodec::type codec,
        const uint8_t* src, uint64_t srcSize, uint8_t* dst, uint64_t dstSize);
    void preparePageV2(koredb_parquet::format::PageHeader& pageHdr);
    void preparePage(koredb_parquet::format::PageHeader& pageHdr);
    void prepareDataPage(koredb_parquet::format::PageHeader& pageHdr);
...
protected:
    const koredb_parquet::format::SchemaElement& schema;
    ...
    const koredb_parquet::format::ColumnChunk* chunk = nullptr;
