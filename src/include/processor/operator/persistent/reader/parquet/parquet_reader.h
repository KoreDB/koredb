...
struct ParquetReaderScanState {
    ...
    std::unique_ptr<koredb_apache::thrift::protocol::TProtocol> thriftFileProto;
...
public:
    ...
    koredb_parquet::format::FileMetaData* getMetadata() const { return metadata.get(); }

private:
    std::unique_ptr<koredb_apache::thrift::protocol::TProtocol> createThriftProtocol(
        common::FileInfo* fileInfo_, bool prefetch_mode) {
        return std::make_unique<
            koredb_apache::thrift::protocol::TCompactProtocolT<ThriftFileTransport>>(
            std::make_shared<ThriftFileTransport>(fileInfo_, prefetch_mode));
    }
    const koredb_parquet::format::RowGroup& getGroup(ParquetReaderScanState& state) {
        KU_ASSERT(
    ...
    static common::LogicalType deriveLogicalType(
        const koredb_parquet::format::SchemaElement& s_ele);
    void initMetadata();
...
private:
    ...
    std::unique_ptr<koredb_parquet::format::FileMetaData> metadata;
    main::ClientContext* context;
};
