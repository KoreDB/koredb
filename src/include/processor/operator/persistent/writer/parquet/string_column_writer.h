...
public:
    StringColumnWriterState(koredb_parquet::format::RowGroup& rowGroup, uint64_t colIdx,
...
public:
    ...
    inline koredb_parquet::format::Encoding::type getEncoding(
        BasicColumnWriterState& writerState) override {
        auto& state = reinterpret_cast<StringColumnWriterState&>(writerState);
        return state.isDictionaryEncoded() ? koredb_parquet::format::Encoding::RLE_DICTIONARY :
                                             koredb_parquet::format::Encoding::PLAIN;
    }
