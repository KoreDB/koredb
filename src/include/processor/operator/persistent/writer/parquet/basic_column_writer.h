...
public:
    BasicColumnWriterState(koredb_parquet::format::RowGroup& rowGroup, uint64_t colIdx)
...
public:
    std::unique_ptr<ColumnWriterState> initializeWriteState(
        koredb_parquet::format::RowGroup& rowGroup) override;
...
protected:
    ...
    virtual koredb_parquet::format::Encoding::type getEncoding(BasicColumnWriterState& /*state*/) {
        return koredb_parquet::format::Encoding::PLAIN;
    }
    ...
    void registerToRowGroup(koredb_parquet::format::RowGroup& rowGroup);
};
