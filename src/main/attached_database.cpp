...
void AttachedDatabase::invalidateCache() {
    if (dbType != common::ATTACHED_KOREDB_DB_TYPE) {
    ...
    if (vfs->fileOrPathExists(walFilePath, context)) {
        ...
        if (walFile->getFileSize() > 0) {
            throw common::RuntimeException(common::stringFormat(
                "Cannot attach an external KoreDB database with non-empty wal file. Try manually "
                "checkpointing the external database (i.e., run \"CHECKPOINT;\")."));
        }
    }
}

AttachedKoreDBDatabase::AttachedKoreDBDatabase(std::string dbPath, std::string dbName,
    ...
    if (DBConfig::isDBPathInMemory(dbPath)) {
        throw common::RuntimeException("Cannot attach an in-memory KoreDB database. Please give a "
                                       "path to an on-disk KoreDB database directory.");
    }
    ...
    if (!vfs->fileOrPathExists(path, clientContext)) {
        throw common::RuntimeException(common::stringFormat(
            "Cannot attach a remote KoreDB database due to invalid path: {}.", path));
    }
