...
using koredb_parquet::format::CompressionCodec;
using koredb_parquet::format::ConvertedType;
using koredb_parquet::format::Encoding;
using koredb_parquet::format::PageType;
using koredb_parquet::format::Type;

ColumnReader::ColumnReader(ParquetReader& reader, LogicalType type,
    const koredb_parquet::format::SchemaElement& schema, idx_t fileIdx, uint64_t maxDefinition,
    uint64_t maxRepeat)
...
void ColumnReader::initializeRead(uint64_t /*rowGroupIdx*/,
    const std::vector<koredb_parquet::format::ColumnChunk>& columns,
    koredb_apache::thrift::protocol::TProtocol& protocol) {
...
std::unique_ptr<ColumnReader> ColumnReader::createReader(ParquetReader& reader,
    common::LogicalType type, const koredb_parquet::format::SchemaElement& schema, uint64_t fileIdx,
...
void ColumnReader::prepareRead(parquet_filter_t& /*filter*/) {
    ...
    defineDecoder.reset();
    block.reset();
    koredb_parquet::format::PageHeader pageHdr;
...
void ColumnReader::decompressInternal(koredb_parquet::format::CompressionCodec::type codec,
    ...
    case CompressionCodec::SNAPPY: {
        {
            ...
            auto res = koredb_snappy::GetUncompressedLength(reinterpret_cast<const char*>(src),
        ...
        auto res = koredb_snappy::RawUncompress(reinterpret_cast<const char*>(src), srcSize,
    ...
    case CompressionCodec::ZSTD: {
        auto res = koredb_zstd::ZSTD_decompress(dst, dstSize, src, srcSize);
        // LCOV_EXCL_START
        if (koredb_zstd::ZSTD_isError(res) || res != (size_t)dstSize) {
    ...
    case CompressionCodec::LZ4_RAW: {
        auto res = koredb_lz4::LZ4_decompress_safe(reinterpret_cast<const char*>(src),
...
void ColumnReader::preparePageV2(koredb_parquet::format::PageHeader& pageHdr) {
...
void ColumnReader::preparePage(koredb_parquet::format::PageHeader& pageHdr) {
    auto& trans = reinterpret_cast<ThriftFileTransport&>(*protocol->getTransport());
...
void ColumnReader::prepareDataPage(koredb_parquet::format::PageHeader& pageHdr) {
