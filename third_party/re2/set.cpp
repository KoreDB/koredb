...
int RE2::Set::Add(const StringPiece& pattern, std::string* error) {
    ...
    RegexpStatus status;
    koredb::regex::Regexp* re = Regexp::Parse(pattern, pf, &status);
    if (re == NULL) {
        if (error != NULL)
            *error = status.Text();
    ...
    koredb::regex::Regexp* m = koredb::regex::Regexp::HaveMatch(n, pf);
    if (re->op() == kRegexpConcat) {
        int nsub = re->nsub();
        PODArray<koredb::regex::Regexp*> sub(nsub + 1);
        for (int i = 0; i < nsub; i++)
            sub[i] = re->sub()[i]->Incref();
        sub[nsub] = m;
        re->Decref();
        re = koredb::regex::Regexp::Concat(sub.data(), nsub + 1, pf);
    } else {
        koredb::regex::Regexp* sub[2];
        sub[0] = re;
        sub[1] = m;
        re = koredb::regex::Regexp::Concat(sub, 2, pf);
    }
...
bool RE2::Set::Compile() {
    ...
    PODArray<koredb::regex::Regexp*> sub(size_);
    ...
    Regexp::ParseFlags pf = static_cast<Regexp::ParseFlags>(options_.ParseFlags());
    koredb::regex::Regexp* re = koredb::regex::Regexp::Alternate(sub.data(), size_, pf);
