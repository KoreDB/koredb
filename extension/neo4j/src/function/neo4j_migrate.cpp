...
static LogicalType inferKoreDBType(nlohmann::json types) {
...
std::pair<std::string, std::string> getCreateNodeTableQuery(httplib::Client& cli,
    ...
    for (const auto& item : data) {
        ...
        auto kuType = inferKoreDBType(item["row"][1]);
...
std::string migrateQuery(ClientContext& /*context*/, const TableFuncBindData& bindData) {
    ...
    outputQuery.append(
        "] as row RETURN row[1] as koredb_table, row[2] as koredb_property, row[3] as "
        "koredb_type, row[4] as neo4j_types;");
    result += outputQuery;
    return result;
}
