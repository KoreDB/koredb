...
with open(os.path.join(base_dir, 'koredb-source', 'tools', 'python_api', 'requirements_dev.txt')) as f:
    requirements = f.read().splitlines()


def _get_koredb_version():
    cmake_file = os.path.join(base_dir, 'koredb-source', 'CMakeLists.txt')
...
print("The version of this build is %s" % koredb_version)
...
class CMakeBuild(build_ext):
    def build_extension(self, ext: CMakeExtension) -> None:
        ...
        build_dir = os.path.join(ext.sourcedir, 'koredb-source')
...
setup(name='koredb',
      version=koredb_version,
      install_requires=[],
      ext_modules=[CMakeExtension(
          name="koredb", sourcedir=base_dir)],
