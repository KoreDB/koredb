...
public class ValueTest extends TestBase {
    ...
    void CreateListLiteral() {
        ...
        KoreDBList list = new KoreDBList(listValues);
    ...
    void CreateListLiteralNested() {
        ...
        KoreDBList[] nestedLists = { new KoreDBList(nestedListValues[0]), new KoreDBList(nestedListValues[1]) };

        Value[] listValues = { nestedLists[0].getValue(), nestedLists[1].getValue() };
        KoreDBList list = new KoreDBList(listValues);
        assertEquals(listValues.length, list.getListSize());
        for (int i = 0; i < list.getListSize(); ++i) {
            KoreDBList nestedList = new KoreDBList(list.getListElement(i));
    ...
    void CreateListDefaultValues() {
        int listLength = 5;
        KoreDBList list = new KoreDBList(new DataType(DataTypeID.INT32), listLength);
    ...
    void ValueGetListSize() {
        ...
        assertFalse(value.isNull());
        KoreDBList list = new KoreDBList(value);
    ...
    void CreateStructLiteral() {
        ...
        KoreDBStruct structVal = new KoreDBStruct(fieldNames, fieldValues);
    ...
    void CreateStructLiteralNested() {
        ...
        KoreDBStruct person = new KoreDBStruct(personFieldNames, personFieldValues);

        String[] companyFieldNames = { "name", "boss" };
        Value[] companyFieldValues = { new Value("Company"), person.getValue() };
        KoreDBStruct company = new KoreDBStruct(companyFieldNames, companyFieldValues);

        KoreDBStruct actualPerson = new KoreDBStruct(company.getValueByFieldName("boss"));
    ...
    void CreateStructLiteralFromMap() {
        Map<String, Value> fields = Map.of(
                ...
                "ID", new Value(2),
                "age", new Value(20));
        KoreDBStruct structVal = new KoreDBStruct(fields);
    ...
    void StructValGetNumFields() {
        ...
        Value value = flatTuple.getValue(0);
        assertTrue(value.isOwnedByCPP());
        KoreDBStruct structVal = new KoreDBStruct(value);
    ...
    void CreateMapLiteral() {
        ...
        KoreDBMap koredbMap = new KoreDBMap(keys, values);
        assertEquals(keys.length, koredbMap.getNumFields());
        int aliceKeyIdx = (koredbMap.getKey(0).getValue().equals("Alice")) ? 0 : 1;
        int bobKeyIdx = 1 - aliceKeyIdx;
        assertEquals("Alice", koredbMap.getKey(aliceKeyIdx).getValue());
        assertEquals(1, (Integer) koredbMap.getValue(aliceKeyIdx).getValue());
        assertEquals("Bob", koredbMap.getKey(bobKeyIdx).getValue());
        assertEquals(2, (Integer) koredbMap.getValue(bobKeyIdx).getValue());

        PreparedStatement stmt = conn
                .prepare("MATCH (m:movies) WHERE m.name = 'Roma' SET m.audience = $audience");
        Map<String, Value> options = Map.of(
                "audience", koredbMap.getValue());
    ...
    void CreateMapLiteralNested() {
        ...
        KoreDBMap map0 = new KoreDBMap(nestedKeys[0], nestedValues[0]);
        KoreDBMap map1 = new KoreDBMap(nestedKeys[1], nestedValues[1]);
        Value[] nestedMaps = { map0.getValue(), map1.getValue() };

        Value[] keys = { new Value(Long.valueOf(0)), new Value(Long.valueOf(1)) };
        KoreDBMap koredbMap = new KoreDBMap(keys, nestedMaps);
        assertEquals(keys.length, koredbMap.getNumFields());

        KoreDBMap map1Actual = new KoreDBMap(koredbMap.getValue(1));
    ...
    void MapValGetNumFields() {
        ...
        Value value = flatTuple.getValue(0);
        assertTrue(value.isOwnedByCPP());
        KoreDBMap mapVal = new KoreDBMap(value);
        ...
        try (QueryResult result = conn
            ...
            try (FlatTuple flatTuple = result.getNext();
                ...
                try (Value nodeListValue = ValueRecursiveRelUtil.getNodeList(value)) {
                    ...
                    KoreDBList nodeList = new KoreDBList(nodeListValue);
                ...
                try (Value relListValue = ValueRecursiveRelUtil.getRelList(value)) {
                    ...
                    KoreDBList relList = new KoreDBList(relListValue);
