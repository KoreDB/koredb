...
public class KoreDBMap implements AutoCloseable {
     ...
     */
    public KoreDBMap(Value[] keys, Value[] values) {
        ...
        mapVal = Native.koredbCreateMap(keys, values);
    }
    ...
    private Value getMapKeyOrValue(long index, boolean isKey) {
        ...
        Value structValue = Native.koredbValueGetListElement(mapVal, index);
        Value keyOrValue = new KoreDBList(structValue).getListElement(isKey ? 0 : 1);
        structValue.close();
        return keyOrValue;
    }
    ...
    public long getNumFields() {
        ...
        return Native.koredbValueGetListSize(mapVal);
    }
