...
pub enum ConversionError {
    /// KoreDB's internal date as the number of days since 1970-01-01
    Date(i32),
    /// KoreDB's internal timestamp as the number of microseconds since 1970-01-01
    Timestamp(i64),
    TimestampTz(i64),
    TimestampNs(i64),
    TimestampMs(i64),
...
impl std::fmt::Debug for ConversionError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        match self {
            Date(days) => write!(f, "Could not convert KoreDB date offset of UNIX_EPOCH + {days} days to time::Date"),
            Timestamp(us) => write!(f, "Could not convert KoreDB timestamp offset of UNIX_EPOCH + {us} microseconds to time::OffsetDateTime"),
            TimestampTz(us) => write!(f, "Could not convert KoreDB timestamp_tz offset of UNIX_EPOCH + {us} microseconds to time::OffsetDateTime"),
            TimestampNs(ns) => write!(f, "Could not convert KoreDB timestamp_ns offset of UNIX_EPOCH + {ns} nanoseconds to time::OffsetDateTime"),
            TimestampMs(ms) => write!(f, "Could not convert KoreDB timestamp_ms offset of UNIX_EPOCH + {ms} milliseconds to time::OffsetDateTime"),
            TimestampSec(sec) => write!(f, "Could not convert KoreDB timestamp_sec offset of UNIX_EPOCH + {sec} seconds to time::OffsetDateTime"),
        }
    }
}
...
/// Data types supported by KoreDB
///
/// Also see <https://koredb.github.io/docs/cypher/data-types>
#[derive(Clone, Debug, PartialEq)]
pub enum Value {
    Null(LogicalType),
    Bool(bool),
    ...
    ///
    /// <https://koredb.github.io/docs/cypher/data-types>
    Date(time::Date),
...
impl std::fmt::Display for Value {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            // Note: These don't match koredb's toString, but we probably don't want them to
    ...
    fn try_from(value: &ffi::Value) -> Result<Self, Self::Error> {
        ...
        match ffi::value_get_data_type_id(value) {
            ...
            LogicalTypeID::INTERVAL => Ok(Value::Interval(time::Duration::new(
                ...
                // Duration is constructed using nanoseconds, but koredb stores microseconds
...
impl TryInto<cxx::UniquePtr<ffi::Value>> for Value {
    ...
    fn try_into(self) -> Result<cxx::UniquePtr<ffi::Value>, Self::Error> {
        ...
        fn date_to_koredb_date_t(value: time::Date) -> i32 {
            // Convert to days since 1970-01-01
            (value - time::Date::from_ordinal_date(1970, 1).unwrap())
                .whole_days()
                .to_i32()
                .expect("koredb_date i64->i32 overflow")
        }
        ...
        match self {
            ...
            Value::Date(value) => Ok(ffi::create_value_date(date_to_koredb_date_t(value))),
...
mod tests {
    ...
    macro_rules! type_tests {
        ...
        $(
            #[test]
            /// Tests that the values are correctly converted into `koredb::common::Value` and back
            fn $name() -> Result<()> {
    ...
    database_tests! {
        // Passing these values as arguments is not yet implemented in koredb:
    ...
    #[test]
    /// Test that the timestamp round-trips through koredb's internal timestamp
