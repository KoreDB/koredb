...
// Check if prebuilt binaries are available
  ...
  fs.copyFileSync(prebuiltPath, path.join(__dirname, "koredbjs.node"));
  console.log(
    `Copied ${prebuiltPath} -> ${path.join(__dirname, "koredbjs.node")}.`
  );
...
childProcess.execSync("npm install", {
  cwd: path.join(__dirname, "koredb-source", "tools", "nodejs_api"),
  stdio: "inherit",
});

// Build the KoreDB source code
console.log("Building KoreDB source code...");
...
childProcess.execSync("make nodejs NUM_THREADS=" + THREADS, {
  env,
  cwd: path.join(__dirname, "koredb-source"),
  stdio: "inherit",
});
