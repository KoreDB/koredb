...
if (TEST_INSTALLED) {
  ...
  global.koredbPath = require.resolve("../build/");
  console.log("Testing locally built version @", koredbPath);
}
...
const initTests = async () => {
  ...
  const db = new koredb.Database(dbPath, 1 << 28 /* 256MB */);
  const conn = new koredb.Connection(db, 4);
