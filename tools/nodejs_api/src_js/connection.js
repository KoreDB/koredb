...
class Connection {
   ...
   *
   * @param {koredb.Database} database the database object to connect to.
  ...
  async init() {
    ...
    if (!this._connection) {
      ...
      this._connection = new KoreDBNative.NodeConnection(database);
    }
   ...
   * @returns {KoreDBNative.NodeConnection} the underlying native connection.
   ...
   * @param {koredb.PreparedStatement} preparedStatement the prepared statement to execute.
   ...
   * @returns {Promise<koredb.QueryResult>} a promise that resolves to the query result. The promise is rejected if there is an error.
   */
  execute(preparedStatement, params = {}, progressCallback) {
    return new Promise((resolve, reject) => {
      ...
      this._getConnection()
        .then((connection) => {
          const nodeQueryResult = new KoreDBNative.NodeQueryResult();
          try {
   ...
   * @param {koredb.PreparedStatement} preparedStatement the prepared statement
   * @param {Object} params a plain object mapping parameter names to values.
   * @returns {Array<koredb.QueryResult> | koredb.QueryResult} an array of query results. If there is only one query result, the function returns the query result directly.
   ...
   * @returns {Promise<koredb.PreparedStatement>} a promise that resolves to the prepared statement. The promise is rejected if there is an error.
   */
  prepare(statement) {
    return new Promise((resolve, reject) => {
      ...
      this._getConnection()
        .then((connection) => {
          const preparedStatement = new KoreDBNative.NodePreparedStatement(
            connection,
            statement
          );
   ...
   * @param {String} statement the statement to prepare. 
   * @returns {koredb.PreparedStatement} the prepared statement.
   * @throws {Error} if there is an error.
   */
   ...
   * @param {KoreDBNative.NodeQueryResult} nodeQueryResult the current node query result.
   * @returns {Promise<koredb.QueryResult>} a promise that resolves to the next query result. The promise is rejected if there is an error.
   */
  _getNextQueryResult(nodeQueryResult) {
    return new Promise((resolve, reject) => {
      const nextNodeQueryResult = new KoreDBNative.NodeQueryResult();
   ...
   * @param {KoreDBNative.NodeQueryResult} nodeQueryResult the node query result.
   * @returns {Promise<Array<koredb.QueryResult>> | koredb.QueryResult} a promise that resolves to an array of query results. The promise is rejected if there is an error.
   */
   ...
   * @param {KoreDBNative.NodeQueryResult} nodeQueryResult the node query result.
   * @returns {Array<koredb.QueryResult> | koredb.QueryResult} an array of query results.
   * @throws {Error} if there is an error.
   */
