...
const KOREDB_ROOT = path.resolve(path.join(__dirname, "..", ".."));
const CURRENT_DIR = path.resolve(__dirname);
const ARCHIVE_PATH = path.resolve(path.join(__dirname, "koredb-source.tar"));
...
(async () => {
  console.log("Gathering KoreDB source code...");
  ...
  // Remove the old koredb-source directory
  try {
    await fs.rm(path.join(CURRENT_DIR, "koredb-source"), { recursive: true });
  } catch (e) {
    // Ignore
  }

  // Create the koredb-source directory
  await fs.mkdir(path.join(CURRENT_DIR, "koredb-source"));

  // Extract the archive to koredb-source
  await new Promise((resolve, reject) => {
    childProcess.execFile(
      "tar",
      ["-xf", ARCHIVE_PATH, "-C", "koredb-source"],
      { cwd: CURRENT_DIR },
      (err) => {
        if (err) {
          reject(err);
  ...
  // Move koredb-source to archive
  await fs.rename(
    path.join(CURRENT_DIR, "koredb-source"),
    path.join(ARCHIVE_DIR_PATH, "koredb-source")
  );
  ...
  await fs.copyFile(
    path.join(KOREDB_ROOT, "README.md"),
  ...
  await fs.unlink(
    path.join(ARCHIVE_DIR_PATH, "koredb-source", "tools", "rust_api", "koredb-src")
  );
  ...
  await new Promise((resolve, reject) => {
    childProcess.execFile(
      "tar",
      ["-czf", "koredb-source.tar.gz", "package"],
      { cwd: CURRENT_DIR },
      (err) => {
        if (err) {
          reject(err);
