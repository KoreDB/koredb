...
class Connection {
   ...
   *
   * @param {koredb.sync.Database} database the database object to connect to.
   * @param {Number} numThreads the maximum number of threads to use for query 
   * execution.
   */
  constructor(database, numThreads = null) {
    KoreDBWasm.checkInit();
    const koredb = KoreDBWasm._koredb;
    ...
    this._connection = new koredb.Connection(database._database);
   ...
   * @returns {koredb.sync.QueryResult} the query result.
   * @throws {Error} if the connection is closed.
   ...
   * @returns {Promise<koredb.sync.PreparedStatement>} the prepared statement.
   */
  prepare(statement) {
    this._checkConnection();
    if (typeof statement !== "string") {
   ...
   * @param {koredb.sync.PreparedStatement} preparedStatement the prepared 
